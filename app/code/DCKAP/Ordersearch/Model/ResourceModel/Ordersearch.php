...
class Ordersearch extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    protected function orderProductFilter($dataObject)
    {
        ...
        $this->orders->getSelect()->joinLeft(
            ...
            [self::ORDER_ITEM . '.sku', self::ORDER_ITEM . '.name']
        );
    ...
    protected function orderAddressFilter($dataObject)
    {
        ...
        $this->orders->getSelect()->joinLeft(
            self::ORDER_ADDRES,
            self::MAIN_TABLE . '.entity_id =' . self::ORDER_ADDRES . '.parent_id',
            [self::ORDER_ADDRES . '.city', self::ORDER_ADDRES . '.postcode']
        );
        ...
        $this->orders
            ->addAttributeToFilter(
                self::ORDER_ADDRES . "." . $dataObject['filter_id'], ['like' => "%" . $dataObject['filter_value'] . "%"]
            )

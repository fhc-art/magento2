...
namespace Bss\CustomizeCmindMultiAccount\Plugin\Controller\Adminhtml\Manage;
...
class Save extends AbstractManage
{
    ...
    public function execute()
    {
        ...
        try {
            ...
            $customerModel = $this->customerRegistry
                ->retrieve($savedSubaccountTransportDataObject->getCustomerId());
            $parentCustomer = $this->customerRegistry
                ->retrieve($parentCustomerId);
            $checkEdit = !empty($subaccountData['id'])
                ? $subaccountData['id']
                : null;
            if (!$checkEdit) {
                //set default bill & shipping address follow parent account And set status b2b approved
                try {
                    $defaultBilling = $parentCustomer->getDefaultBillingAddress();
                    ...
                    $address->save();
                    $defaultShipping = $parentCustomer->getDefaultShippingAddress();
                    ...
                    $customerModel
                        ->setData('ca_resale_number', $parentCustomer->getData('ca_resale_number'))
                        ->getResource()
                        ->saveAttribute($customerModel, 'ca_resale_number');
                    $customerModel
                        ->setData('ca_customer_number', $parentCustomer->getData('ca_customer_number'))
                        ->getResource()

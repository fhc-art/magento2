...
namespace Bss\CustomizeCmindMultiAccount\Plugin\Controller\Manage;
...
class EditPost extends AbstractManage
{
    ...
    public function execute()
    {
        ...
        try {
            ...
            if ($this->viewHelper->isSubaccountLoggedIn()) {
                $currentSubaccountData = $this->subaccountTransportRepository->getByCustomerId($customerId);
            ...
            if (!$checkEdit) {
                //set default bill & shipping address follow parent account And set status b2b approved
                try {
                    $defaultBilling = $this->getSession()->getCustomer()->getDefaultBillingAddress();
                    $address = $this->addressFactory->create();
                    $address->setCustomerId($savedSubaccountTransportDataObject->getCustomerId())
                        ->setFirstname($defaultBilling->getFirstname())
                        ->setLastname($defaultBilling->getLastname())
                        ->setCountryId($defaultBilling->getCountryId())
                        ->setRegionId($defaultBilling->getRegionId())
                        ->setRegion($defaultBilling->getRegion())
                        ->setPostcode($defaultBilling->getPostcode())
                        ->setCity($defaultBilling->getCity())
                        ->setTelephone($defaultBilling->getTelephone())
                        ->setFax($defaultBilling->getFax())
                        ->setCompany($defaultBilling->getCompany())
                        ->setStreet($defaultBilling->getStreet())
                        ->setIsDefaultBilling('1')
                        ->setSaveInAddressBook('1');
                    $address->save();
                    $defaultShipping = $this->getSession()->getCustomer()->getDefaultShippingAddress();
                    $shippingAddress = $this->addressFactory->create();
                    $shippingAddress->setCustomerId($savedSubaccountTransportDataObject->getCustomerId())
                        ->setFirstname($defaultShipping->getFirstname())
                        ->setLastname($defaultShipping->getLastname())
                        ->setCountryId($defaultShipping->getCountryId())
                        ->setRegionId($defaultShipping->getRegionId())
                        ->setRegion($defaultShipping->getRegion())
                        ->setPostcode($defaultShipping->getPostcode())
                        ->setCity($defaultShipping->getCity())
                        ->setTelephone($defaultShipping->getTelephone())
                        ->setFax($defaultShipping->getFax())
                        ->setCompany($defaultShipping->getCompany())
                        ->setStreet($defaultShipping->getStreet())
                        ->setIsDefaultShipping('1')
                        ->setSaveInAddressBook('1');
                    $shippingAddress->save();
                    $customerModel
                        ->setData('b2b_activasion_status', 2)
                        ->getResource()
                        ->saveAttribute($customerModel, 'b2b_activasion_status');
                    $customerModel
                        ->setData('ca_resale_number', $this->session->getCustomer()->getData('ca_resale_number'))
                        ->getResource()
                        ->saveAttribute($customerModel, 'ca_resale_number');
                    $customerModel
                        ->setData('ca_customer_number', $this->session->getCustomer()->getData('ca_customer_number'))
                        ->getResource()
                        ->saveAttribute($customerModel, 'ca_customer_number');
     ...
     *
     * @return \Cminds\MultiUserAccounts\Controller\Manage\EditPost
     * @throws \Magento\Framework\Exception\LocalizedException
     * @throws \Magento\Framework\Exception\NoSuchEntityException

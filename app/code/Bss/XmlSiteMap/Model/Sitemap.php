...
class Sitemap extends \Magento\Framework\Model\AbstractModel
{
    ...
    {
        $this->_init('Bss\XmlSiteMap\Model\ResourceModel\Sitemap');
    }
    ...
    public function generateXml()
    {
        ...
        if ($this->isEnabledSubmissionRobots()) {
            $this->addSitemapToRobotsTxt($this->getData('xml_sitemap_filename'));
        }
    ...
    public function createSitemapIndex()
    {
        ...
        for ($i = 1; $i <= $this->sitemapIncrement; $i++) {
            $xml = $this->getSitemapIndexRow($this->getCurrentSitemapFilename($i), $this->processVariable->getCurrentDateTime());
            $this->writeSitemapRow($xml);
        }
    ...
    public function getSitemapRow($url, $lastmod = null, $changefreq = null, $priority = null, $images = null)
    {
        ...
        if ($images) {
            ...
            foreach ($images->getCollection() as $image) {
                ...
                $row .= '<image:image>';
                $row .= '<image:loc>' . htmlspecialchars($this->getMediaUrl($image->getUrl())) . '</image:loc>';
    ...
    {
        return realpath($this->processVariable->getRequest()->getServer('DOCUMENT_ROOT'));
    }
    ...
    protected function getStoreBaseDomain()
    {
        ...
        $storeParsedUrl = parse_url($this->getStoreBaseUrl());
        // @codingStandardsIgnoreEnd
        $url = $storeParsedUrl['scheme'] . '://' . $storeParsedUrl['host'];

        $documentRoot = trim(str_replace('\\', '/', $this->getDocumentRoot()), '/');
        $baseDir = trim(str_replace('\\', '/', $this->getBaseDir()), '/');
        ...
        if (strpos($baseDir, $documentRoot) === 0) {
            ...
            $url = $this->getStoreBaseUrl(UrlInterface::URL_TYPE_WEB);
     ...
     * @return bool
     */
    public function isEnabledSubmissionRobots()
    {
        /** @var $helper \Bss\XmlSiteMap\Helper\Data */
        $helper = $this->sitemapData;
        $storeId = $this->getStoreId();
     ...
     * @throws \Magento\Framework\Exception\ValidatorException
     */
    public function addSitemapToRobotsTxt($sitemapFileName)
    {
        $robotsSitemapLine = 'Sitemap: ' . $this->getSitemapUrl($this->getData('xml_sitemap_path'), $sitemapFileName);

        $rootPath = $this->sitemapData->getRootPath();
        $filename = $rootPath . 'robots.txt';
        $content = '';
        if ($this->processVariable->getDirectory()->isExist($filename)) {
            $content = $this->processVariable->getDirectory()->readFile($filename);
        }

        if (strpos($content, $robotsSitemapLine) === false) {
            if (!empty($content)) {
                $content .= $this->findNewLinesDelimiter($content);
            }
            $content .= $robotsSitemapLine;
        }

        $this->processVariable->getDirectory()->writeFile($filename, $content);
    }

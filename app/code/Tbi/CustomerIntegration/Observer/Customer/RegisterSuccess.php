...
class RegisterSuccess implements \Magento\Framework\Event\ObserverInterface
{
     ...
     * @param Logger               $logger
     */
    public function __construct(ZendClientFactory $httpClientFactory, ScopeConfigInterface $config, CustomerRepositoryInterface $customerRepository, \Psr\Log\LoggerInterface $logger, \Magento\Customer\Model\SessionFactory $customerSession)
    {
    ...
    public function execute(
        ...
        if ($customer->getId()) {
            ...
            try {
                ...
                $this->logger->info("RegisterSuccess");
                ...
                if ($response->getStatus() == '200') {
                    $this->updateCustomerAttribute($customer, 'ca_api_data', '1');
                } else {
                    $this->updateCustomerAttribute($customer, 'ca_api_data', '0');
                }
            } catch (\Exception $e) {
    ...
    public function updateCustomerAttribute($customer, $attributeCode, $value)
    {
        try {
            $customerDetails = $customer;
            $customerDetails->setCustomAttribute($attributeCode, $value);
            $this->customerSession->create()->setBssSaveAccount('true');

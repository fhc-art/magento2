...
define([
    ...
    $.widget('bss.Sdcp', $.mage.SwatchRenderer, {
        ...
        _UpdateStock: function ($status, $number, $config) {
            ...
            if (!$number && window.isBackorderGlobal) {
                $('.product-info-price .stock').css('display', 'none');
            } else {
            	$('.product-info-price .stock').css('display', 'block');
            }
        },
                ...
                if (typeof $vl === 'string') {
                    ...
                    try {
                        if ($('.swatch-attribute[attribute-code="'
                            ...
                            $('.swatch-attribute[attribute-code="' + $code + '"] .swatch-attribute-options .swatch-option').each(function () {
                                var optionLable = $(this).attr('option-label').replace(/[~`!@#$%^&*()_|\;:'",.<>\{\}\[\]\\\/]/g,'').replace(/\s/g,'');
